 ...
 * ================================================
 */
public class BookDetailHelpActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initToolBar() {
        toolbarTitle.setText("书荒互助区详情页面");
    }
        ...
        adapter.setMore(R.layout.view_recycle_more, new RecyclerArrayAdapter.OnMoreListener() {
            ...
            public void onMoreShow() {
                if (NetworkUtils.isConnected()) {
                    ...
                    adapter.pauseMore();
                    Toast.makeText(BookDetailHelpActivity.this, "网络不可用", Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void AddHeader() {
        ...
        adapter.addHeader(new RecyclerArrayAdapter.ItemView() {
            ...
            public View onCreateView(ViewGroup parent) {
                View view = LayoutInflater.from(BookDetailHelpActivity.this).inflate(R.layout.header_book_discussion_detail, parent, false);
                return view;
            }
     ...
     * 获取书荒区帖子详情
     */
    private void getBookHelpDetail(String id) {
        BookReaderModel model = BookReaderModel.getInstance(this);
        Subscription rxSubscription = model.getBookHelpDetail(id)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<DetailBookHelp>() {
                    @Override
                    ...
                    @Override
                    public void onNext(DetailBookHelp content) {
                        if(content!=null && content.isOk()){
                            DetailBookHelp.HelpBean help = content.getHelp();
                            ImageUtils.loadImgByPicassoPerson(BookDetailHelpActivity.this,
                                    ConstantZssqApi.IMG_BASE_URL+help.getAuthor().getAvatar(),
                                    R.drawable.avatar_default,
                                    headerViewHolder.ivAvatar);
                            headerViewHolder.tvNickName.setText(help.getAuthor().getNickname());
                            headerViewHolder.tvTime.setText(FormatUtils.getDescriptionTimeFromDateString(help.getCreated()));
                            headerViewHolder.tvTitle.setText(help.getTitle());
                            headerViewHolder.tvContent.setText(help.getContent());
                            headerViewHolder.tvCommentCount.setText(String.format(BookDetailHelpActivity.this.getString(R.string.comment_comment_count), help.getCommentCount()));
                        }
                    }
                });
    }
    ...
    private void getBestComments(String id) {
        BookReaderModel model = BookReaderModel.getInstance(BookDetailHelpActivity.this);
                    ...
                    public void onNext(DetailDiscussionComment list) {
                        if(list!=null && list.isOk()){
                            if (list.getComments().isEmpty()) {
                                ...
                                headerViewHolder.rvBestComments.setLayoutManager(new LinearLayoutManager(BookDetailHelpActivity.this));
                                RecycleViewItemLine line = new RecycleViewItemLine(BookDetailHelpActivity.this, LinearLayout.HORIZONTAL,
                                        SizeUtils.dp2px(1), Color.parseColor("#f5f5f7"));
                                headerViewHolder.rvBestComments.addItemDecoration(line);
                                mBestCommentListAdapter = new BookDetailCommentAdapter(BookDetailHelpActivity.this, mBestCommentList);
     ...
     *  获取书评区、书荒区帖子详情内的评论列表
     */
    private void getBookHelpComments(String id, final int start, final int limit) {
        BookReaderModel model = BookReaderModel.getInstance(BookDetailHelpActivity.this);
        Subscription rxSubscription = model.getBookReviewComments(id, start + "", limit + "")
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<DetailDiscussionComment>() {
                    @Override
                    ...
                    public void onNext(DetailDiscussionComment list) {
                        if(list!=null && list.isOk()){
                            if(adapter==null){
                                adapter = new BookDetailDiscussionAdapter(BookDetailHelpActivity.this);
                            }
